...
| <a name="input_cluster_capacity_providers_wait_duration"></a> [cluster\_capacity\_providers\_wait\_duration](#input\_cluster\_capacity\_providers\_wait\_duration) | The amount of time to wait for the cluster capacity providers to be associated with the cluster. This is useful when creating capacity providers in the same module as the cluster, as it can take a few seconds for the capacity provider to be ready to associate with the cluster | `string` | `"20s"` | no |
