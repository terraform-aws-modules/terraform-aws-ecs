...
| <a name="input_create_task_def"></a> [create\_task\_def](#input\_create\_task\_def) | Determines whether to create a task definition or use existing/provided | `bool` | `true` | no |
...
| <a name="input_task_def_container_definitions"></a> [task\_def\_container\_definitions](#input\_task\_def\_container\_definitions) | A list of valid [container definitions](http://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_ContainerDefinition.html) provided as a single valid JSON document. Please note that you should only provide values that are part of the container definition document | `string` | `""` | no |
| <a name="input_task_def_cpu"></a> [task\_def\_cpu](#input\_task\_def\_cpu) | Number of cpu units used by the task. If the `task_requires_compatibilities` is `FARGATE` this field is required | `number` | `null` | no |
| <a name="input_task_def_ephemeral_storage"></a> [task\_def\_ephemeral\_storage](#input\_task\_def\_ephemeral\_storage) | The amount of ephemeral storage to allocate for the task. This parameter is used to expand the total amount of ephemeral storage available, beyond the default amount, for tasks hosted on AWS Fargate | `any` | `{}` | no |
| <a name="input_task_def_family"></a> [task\_def\_family](#input\_task\_def\_family) | A unique name for your task definition | `string` | `null` | no |
| <a name="input_task_def_inference_accelerator"></a> [task\_def\_inference\_accelerator](#input\_task\_def\_inference\_accelerator) | Configuration block(s) with Inference Accelerators settings | `any` | `{}` | no |
| <a name="input_task_def_ipc_mode"></a> [task\_def\_ipc\_mode](#input\_task\_def\_ipc\_mode) | IPC resource namespace to be used for the containers in the task The valid values are `host`, `task`, and `none` | `string` | `null` | no |
| <a name="input_task_def_memory"></a> [task\_def\_memory](#input\_task\_def\_memory) | Amount (in MiB) of memory used by the task. If the `task_requires_compatibilities` is `FARGATE` this field is required | `number` | `null` | no |
| <a name="input_task_def_network_mode"></a> [task\_def\_network\_mode](#input\_task\_def\_network\_mode) | Docker networking mode to use for the containers in the task. Valid values are `none`, `bridge`, `awsvpc`, and `host` | `string` | `null` | no |
| <a name="input_task_def_pid_mode"></a> [task\_def\_pid\_mode](#input\_task\_def\_pid\_mode) | Process namespace to use for the containers in the task. The valid values are `host` and `task` | `string` | `null` | no |
| <a name="input_task_def_placement_constraints"></a> [task\_def\_placement\_constraints](#input\_task\_def\_placement\_constraints) | Configuration block for rules that are taken into consideration during task placement (up to max of 10) | `any` | `{}` | no |
| <a name="input_task_def_proxy_configuration"></a> [task\_def\_proxy\_configuration](#input\_task\_def\_proxy\_configuration) | Configuration block for the App Mesh proxy | `any` | `{}` | no |
| <a name="input_task_def_requires_compatibilities"></a> [task\_def\_requires\_compatibilities](#input\_task\_def\_requires\_compatibilities) | Set of launch types required by the task. The valid values are `EC2` and `FARGATE` | `list(string)` | `[]` | no |
