...
module "container_definition" {
  ...
  for_each = toset(keys(var.container_definitions))

  operating_system_family = try(var.runtime_platform.operating_system_family, "LINUX")

  # Container Definition
  command                  = try(var.container_definitions[each.key].command, var.container_definition_defaults.command, [])
  cpu                      = try(var.container_definitions[each.key].cpu, var.container_definition_defaults.cpu, null)
  dependencies             = try(var.container_definitions[each.key].dependencies, var.container_definition_defaults.dependencies, []) # depends_on is a reserved word
  disable_networking       = try(var.container_definitions[each.key].disable_networking, var.container_definition_defaults.disable_networking, null)
  dns_search_domains       = try(var.container_definitions[each.key].dns_search_domains, var.container_definition_defaults.dns_search_domains, [])
  dns_servers              = try(var.container_definitions[each.key].dns_servers, var.container_definition_defaults.dns_servers, [])
  docker_labels            = try(var.container_definitions[each.key].docker_labels, var.container_definition_defaults.docker_labels, {})
  docker_security_options  = try(var.container_definitions[each.key].docker_security_options, var.container_definition_defaults.docker_security_options, [])
  enable_execute_command   = try(var.container_definitions[each.key].enable_execute_command, var.container_definition_defaults.enable_execute_command, var.enable_execute_command)
  entrypoint               = try(var.container_definitions[each.key].entrypoint, var.container_definition_defaults.entrypoint, [])
  environment              = try(var.container_definitions[each.key].environment, var.container_definition_defaults.environment, [])
  environment_files        = try(var.container_definitions[each.key].environment_files, var.container_definition_defaults.environment_files, [])
  essential                = try(var.container_definitions[each.key].essential, var.container_definition_defaults.essential, null)
  extra_hosts              = try(var.container_definitions[each.key].extra_hosts, var.container_definition_defaults.extra_hosts, [])
  firelens_configuration   = try(var.container_definitions[each.key].firelens_configuration, var.container_definition_defaults.firelens_configuration, {})
  health_check             = try(var.container_definitions[each.key].health_check, var.container_definition_defaults.health_check, {})
  hostname                 = try(var.container_definitions[each.key].hostname, var.container_definition_defaults.hostname, null)
  image                    = try(var.container_definitions[each.key].image, var.container_definition_defaults.image, null)
  interactive              = try(var.container_definitions[each.key].interactive, var.container_definition_defaults.interactive, false)
  links                    = try(var.container_definitions[each.key].links, var.container_definition_defaults.links, [])
  linux_parameters         = try(var.container_definitions[each.key].linux_parameters, var.container_definition_defaults.linux_parameters, {})
  log_configuration        = try(var.container_definitions[each.key].log_configuration, var.container_definition_defaults.log_configuration, {})
  memory                   = try(var.container_definitions[each.key].memory, var.container_definition_defaults.memory, null)
  memory_reservation       = try(var.container_definitions[each.key].memory_reservation, var.container_definition_defaults.memory_reservation, null)
  mount_points             = try(var.container_definitions[each.key].mount_points, var.container_definition_defaults.mount_points, [])
  name                     = try(var.container_definitions[each.key].name, each.key)
  port_mappings            = try(var.container_definitions[each.key].port_mappings, var.container_definition_defaults.port_mappings, [])
  privileged               = try(var.container_definitions[each.key].privileged, var.container_definition_defaults.privileged, false)
  pseudo_terminal          = try(var.container_definitions[each.key].pseudo_terminal, var.container_definition_defaults.pseudo_terminal, false)
  readonly_root_filesystem = try(var.container_definitions[each.key].readonly_root_filesystem, var.container_definition_defaults.readonly_root_filesystem, true)
  repository_credentials   = try(var.container_definitions[each.key].repository_credentials, var.container_definition_defaults.repository_credentials, {})
  resource_requirements    = try(var.container_definitions[each.key].resource_requirements, var.container_definition_defaults.resource_requirements, [])
  secrets                  = try(var.container_definitions[each.key].secrets, var.container_definition_defaults.secrets, [])
  start_timeout            = try(var.container_definitions[each.key].start_timeout, var.container_definition_defaults.start_timeout, 30)
  stop_timeout             = try(var.container_definitions[each.key].stop_timeout, var.container_definition_defaults.stop_timeout, 120)
  system_controls          = try(var.container_definitions[each.key].system_controls, var.container_definition_defaults.system_controls, [])
  ulimits                  = try(var.container_definitions[each.key].ulimits, var.container_definition_defaults.ulimits, [])
  user                     = try(var.container_definitions[each.key].user, var.container_definition_defaults.user, 0)
  volumes_from             = try(var.container_definitions[each.key].volumes_from, var.container_definition_defaults.volumes_from, [])
  working_directory        = try(var.container_definitions[each.key].working_directory, var.container_definition_defaults.working_directory, null)

  # CloudWatch Log Group
  service                                = var.name
  enable_cloudwatch_logging              = try(var.container_definitions[each.key].enable_cloudwatch_logging, var.container_definition_defaults.enable_cloudwatch_logging, true)
  create_cloudwatch_log_group            = try(var.container_definitions[each.key].create_cloudwatch_log_group, var.container_definition_defaults.create_cloudwatch_log_group, true)
  cloudwatch_log_group_name              = try(var.container_definitions[each.key].cloudwatch_log_group_name, var.container_definition_defaults.cloudwatch_log_group_name, null)
  cloudwatch_log_group_use_name_prefix   = try(var.container_definitions[each.key].cloudwatch_log_group_use_name_prefix, var.container_definition_defaults.cloudwatch_log_group_use_name_prefix, false)
  cloudwatch_log_group_retention_in_days = try(var.container_definitions[each.key].cloudwatch_log_group_retention_in_days, var.container_definition_defaults.cloudwatch_log_group_retention_in_days, 14)
  cloudwatch_log_group_kms_key_id        = try(var.container_definitions[each.key].cloudwatch_log_group_kms_key_id, var.container_definition_defaults.cloudwatch_log_group_kms_key_id, null)
