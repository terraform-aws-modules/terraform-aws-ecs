...
| <a name="input_restart_policy"></a> [restart\_policy](#input\_restart\_policy) | Container restart policy; helps overcome transient failures faster and maintain task availability | <pre>object({<br>    enabled              = bool<br>    ignoredExitCodes     = optional(list(number))<br>    restartAttemptPeriod = optional(number)<br>  })</pre> | <pre>{<br>  "enabled": false<br>}</pre> | no |
