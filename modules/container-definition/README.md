...
| <a name="input_health_check"></a> [health\_check](#input\_health\_check) | The container health check command and associated configuration parameters for the container. See [HealthCheck](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_HealthCheck.html) | <pre>object({<br>    command     = list(string)<br>    interval    = optional(number, 30)<br>    retries     = optional(number, 3)<br>    startPeriod = optional(number)<br>    timeout     = optional(number, 5)<br>  })</pre> | `null` | no |
